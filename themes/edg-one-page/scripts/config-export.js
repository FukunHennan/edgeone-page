...
hexo.extend.helper.register("export_config", function () {
  ...
  // Load all language packs for client-side language switching
  const allLangs = {};
  try {
    const langFiles = fs.readdirSync(languageDir).filter((f) => f.endsWith(".yml"));
    for (const lf of langFiles) {
      const langKey = lf.replace(".yml", "");
      const raw = fs.readFileSync(path.join(languageDir, lf), "utf8");
      try {
        allLangs[langKey] = yaml.load(raw);
      } catch (e) {
        hexo.log.warn(ensurePrefix(`Failed to parse language file ${lf}: ${e}`));
      }
    }
  } catch (e) {
    hexo.log.warn(ensurePrefix(`Failed to load language packs: ${e}`));
  }
